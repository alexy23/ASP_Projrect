...
{
    public class BaseMethods: IAuthorization, IAdminTools
    {
        public string HashFunction(string password, string Salt)
        {
            string NewPassword = Salt + password + Salt;
            GOST Crypt = new GOST(256);
            byte[] bytePass = new byte[NewPassword.Length];
            for (int i = 0; i < bytePass.Length; i++)
            {
                bytePass[i] = Convert.ToByte(NewPassword[i]);
            }
            byte[] res = Crypt.GetHash(bytePass);
            string h256 = BitConverter.ToString(res);
            h256 = h256.Split(new char[] { '-' }, StringSplitOptions.RemoveEmptyEntries).ToString();
            return h256;
        }
        class GOST
        {
        ...
        {
            string chars = "abdefhiknrstyzABDEFGHKNQRSTYZ23456789";
            int numChars = chars.Length;
            string Temp = "";
            Random charGen = new Random();
            for (int i = 0; i < Length; i++)
            {
                Temp += chars.Substring(charGen.Next(1, numChars) - 1, 1);
            }
            return Temp;
        }
    }
